    ...
    public static class Patch_MainButtonWorker
    {
        ...
        public static class DoButton
        {
            ...
            private static float AdjustedLeftMargin(float margin, MainButtonWorker instance, Rect rect)
            {
                ...
                if (Utility.Helper.CanDrawIconAndLabel(rect.width, instance.def))
                {
                    margin += Utility.Helper.CalculatedMainButtonIconMargin(rect.width) + Utility.Helper.MainButtonWorker_IconSize;
                }
            ...
            private static string NoBlankText(string blank, string label, MainButtonWorker instance, Rect rect)
            {
                ...
                if (UIManager.mainButtonMode == Utility.MainButtonRenderMode.TextOnly || Utility.Helper.CanDrawIconAndLabel(rect.width, instance.def))
                    return label;
            ...
            private static Vector2 AdjustedLogoPos(MainButtonWorker instance, Vector2 logoPos, Rect rect)
            {
                ...
                if (Utility.Helper.CanDrawIconAndLabel(rect.width, instance.def))
                {
                    logoPos.x = rect.xMin + Utility.Helper.CalculatedMainButtonIconMargin(rect.width);
                }
